...
namespace ArcadeJets
{
   public class HeadsUpDisplay : MonoBehaviour
   {
      ...
      public TMPro.TextMeshProUGUI braking;


      const float kProjectionDistance = 500.0f;

        Camera_Follow_Controller cameraFollow_;
      ...
      void Start()
      {
         ...
         if (throttle == null)
            ...
            cameraFollow_ = GameObject.FindGameObjectWithTag("Player").GetComponent<Camera_Follow_Controller>();

            DeactivateAeroBrakesText(); //Así no lo tengo que ocultar en el editor
      ...
      void Update()
      {
         if (plane != null)
         {
            ...
            if (cross != null)
            {
               ...
               pos =  cameraFollow_.CurrentCamera.WorldToScreenPoint(plane.transform.position + (plane.transform.forward.normalized * kProjectionDistance));
               //pos.z = 0.0f;
            ...
            if (fpm != null)
            {
               // Put the cross some meters in front of the plane. This way the cross and FPM line up
               // correctly when there is zero angle of attack.
               pos = cameraFollow_.CurrentCamera.WorldToScreenPoint(plane.transform.position + (plane.velocity.normalized * kProjectionDistance));
               //pos.z = 0.0f;
        ...
        public void ActivateAeroBrakesText()
        {
            braking.gameObject.SetActive(true);
        }
        public void DeactivateAeroBrakesText()
        {
            braking.gameObject.SetActive(false);
        }
   }
}
